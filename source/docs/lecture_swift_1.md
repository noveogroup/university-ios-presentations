...
* Инициализатор не возвращает значение. Его задача удостовериться, что объект верно сконфигурирован и готов к работе.
* Порой случается, что нет возможности корректно подготовить объект к работе. Например, создать целое число по строке `"not a number"`. Для обработки таких случаев в `Swift` ввели возможность реализации негарантирующего инициализатора.
